...
Dispatcharr-proxy is a Rust MPEG-TS streaming proxy that sits between upstream stream sources and downstream clients. It multiplexes a single upstream connection to many clients via `tokio::sync::broadcast`.
...
- **`control.rs`** — Control API handlers (PUT/DELETE channels, PUT accounts, POST sync). A Django backend pushes routing config here.
...
1. Django pushes channel routing via `/control/v1/sync` or individual PUT endpoints
